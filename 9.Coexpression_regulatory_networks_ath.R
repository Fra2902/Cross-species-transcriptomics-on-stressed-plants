...
gene_descr <- gene_descr %>%
  ...
  add_row(plaza %>% filter(!(X.gene_id %in% gene_descr$X.gene_id), id_type == "tid") %>% select(-id_type) %>% mutate(id = str_remove(id, "\\.[0-9]+$")))

# extract the vector of genes associated to "response to water"
...
for (tissue in c("leaf", "root")) {
  ...
  coldata <- read.xlsx(here("Project/Arabidopsis thaliana/Data/metadata_filtered_3.xlsx"))
  ...
  coldata <- coldata %>% filter(Major.Category == tissue)
  ...
  for (colour in unique(net$genes_and_modules$Modules)) {
    ...
    gost <- gprofiler2::gost(query = genes, organism = gmt_token, highlight = F, evcodes = T)
  ...
  if (tissue == "leaf") {
    ...
    conserved_modules <- c("darkorange")
  }
  ...
  for (colour in conserved_modules) {
    ...
    edges_filtered_sft <- edges_filtered_sft %>%
      ...
      group_by(Var) %>%
      summarise(id = if (dplyr::n_distinct(id, na.rm = TRUE) == 1) {
          dplyr::first(id)
        } else {
          paste(unique(id), collapse = "/")
        },
        across(-id, dplyr::first)
      )
