...
for(sp in species) {
  ...
  for (tissue in c("leaf", "root")) {
    counts <- read.table(here(paste0("Project/", sp, "/Data/counts_gene_level_", tissue, "_normalised_adjusted.txt")))
...
write.xlsx(df_boxplot, here("Project/general_deg_number.xlsx"))
