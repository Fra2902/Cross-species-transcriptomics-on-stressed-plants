...
generic_BP <- c("GO:0006139", "GO:0006725", "GO:0006807", "GO:0008150", "GO:0008152", "GO:0009058", "GO:0009059", "GO:0009987", "GO:0010467", "GO:0019538", "GO:0034641", "GO:0043170", "GO:0044237", "GO:0044238", "GO:0044249", "GO:0044271", "GO:0046483", "GO:0050789", "GO:0050794", "GO:0050896", "GO:0065007", "GO:0071704", "GO:0090304", "GO:1901360", "GO:1901564", "GO:1901576")
...
for(sp in species) {
  ...
  for (tissue in tissues) {
    ...
    for (expr in c("OVER", "UNDER")) {
      ...
      gost <- gprofiler2::gost(query = genes, organism = gmt_token, highlight = F, custom_bg = expressed_genes_adj, evcodes = T)
...
heatmap_df <- data.frame(terms = conserved_terms)
for (sp in species) {
  for (tissue in tissues) {
    expression <- list()
    terms_list <- setNames(
      lapply(c("OVER", "UNDER"), function(expr) {
        file_path <- here(paste0(
          "Project/", sp, "/Products/Enrichment analysis/",
          "enrichment_analysis_degs_", tissue, "_", expr, ".xlsx"
        ))
        ...
        if (!file.exists(file_path)) {
          ...
          enr <- read.xlsx(file_path)
          enr <- enr %>% dplyr::filter(Description %in% conserved_terms)
    ...
    if (!is_null(terms_list$OVER) & !is.null(terms_list$UNDER)) {
      ...
      } else if (!is_null(terms_list$UNDER)) {
...
heatmap_df <- heatmap_df %>% column_to_rownames("terms")
